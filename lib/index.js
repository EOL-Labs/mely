...
var server = Hapi.createServer(serverConfig.config.hostname, serverConfig.config.port, serverConfig.config.options);
	...
	server.start(function(){
		...
		Mely.Administrator.getSystemCount({
			...
			if (count === 0){
				...
				console.log("Admin Link: " + serverConfig.config.hostname + ":" + serverConfig.config.port + "/admin");
				console.log("Site Link: " + serverConfig.config.hostname + ":" + serverConfig.config.port + "/");
			}
		});
	});
});