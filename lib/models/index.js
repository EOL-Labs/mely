...
module.exports.init = function(virt_modules, done) {
    ...
    (function(model) {
		...
		async.parallel({
			...
			comment: function(cb) {
				model.Comment.sync(syncParams).success(function() {
					cb();
				}).error(function(error) { console.log('Error during Comment.sync(): ' + error); });
			},
			theme: function(cb) {
		...
		}, function (err, resultMelySync) {
			if(err){
				throw err;
			}
			if(resultMelySync !== undefined){
				done();
			}
		});
    })(module.exports);
