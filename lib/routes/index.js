...
var mailTransport = nodemailer.createTransport(mail.mailconfig.method,mail.mailconfig.sendmail.bin);
						...
						Mely.Administrator.createTheme({
							...
							Mely.Administrator.activateTheme({
								...
								if(theme !== undefined){
									...
									var message = {
										from: mail.mailconfig.sendmail.from,
			...
			Mely.Administrator.getPost({
				...
				for(var post in posts){
					if (posts.hasOwnProperty(post)){
						...
						row.comments_allowed = posts[post].comments_allowed;
	...
	config:{
		handler: function(request, reply){
			Mely.Administrator.approveComment({
				...
				reply().redirect("admin/post/comment");
			});			
		}
	}
},
		...
		handler: function(request, reply){
			...
			Mely.Administrator.getTheme({
				...
				Mely.Administrator.getThemeSetting({
					...
					setting.headerimage = themesetting[0].headerfilename;
